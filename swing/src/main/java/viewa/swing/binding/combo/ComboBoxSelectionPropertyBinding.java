package viewa.swing.binding.combo;
 ...
 * @param <TP>
 */
public class ComboBoxSelectionPropertyBinding<SS extends JComboBox, SP, TS, TP> extends
	 ...
	 */
	public ComboBoxSelectionPropertyBinding(SS source, Property<SP> sourceProperty,
			BeanAdapter<TS> target, Property<TP> targetProperty) {
		super(new BasicBeanAdapter<SS>(source),
				sourceProperty, target, targetProperty);
		this.setSourceListener(new ComboBoxSelectionBindListener<TS, TP, SP>(
				this.getTarget(), this.getTargetProperty(), this
						.getSourceProperty()));
	}
	 ...
	 */
	public void bind() {
		this.comboModel = getWorkingComboBoxAdapter(this.getSource()
				.getSource(), modelList);
