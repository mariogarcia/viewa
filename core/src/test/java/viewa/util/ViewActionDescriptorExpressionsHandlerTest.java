...
public class ViewActionDescriptorExpressionsHandlerTest extends TestCase {
	...
	private static final List<String> actionDescriptorsAsXPath = Arrays.asList(
			"/rootNode[@text='rootNode']",
			"/rootNode/files[@text='files']",
			"/rootNode/config[@text='config']");
	...
	public void testGetXmlFromList() throws Exception {
		...
		assertEquals(document.getElementsByTagName("files").item(0)
				.getAttributes().getNamedItem("text").getTextContent(), "files");
	}
